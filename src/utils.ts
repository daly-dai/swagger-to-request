...
export const capitalizedWord = (word: string): string => word.charAt(0).toUpperCase() + word.slice(1)
...
export const getType = (typeObj?: IntegratedType | VoProp): string | undefined => {
  if (typeObj?.type) {
    const typeTmp = CONVERT_TYPE_MAP[typeObj.type as keyof typeof CONVERT_TYPE_MAP]
